...
test("can parse path", () => {
  ...
  assert.equal(serialize(new DeviceUrl(protocol + host + port + path)), "serial,,,hostname,8000,/path/to/place");
  ...
  assert.equal(serialize(new DeviceUrl(protocol + user + host + path)), "serial,user,pass,hostname,,/path/to/place");
  assert.equal(
    serialize(new DeviceUrl(protocol + user + host + port + path)),
